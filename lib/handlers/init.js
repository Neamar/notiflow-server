    ...
    function registerOrUpdateUser(user, cb) {
      if(!user) {
        ...
        if(user.gcmToken !== req.params.gcm_token) {
          console.log("Updating existing user", user.nick, "flowdock: " + req.params.gcm_token, ", GCM: " + req.params.gcm_token);
          ...
          // We register a new listener, the old one will be gc'd by Heroku on the next dyno cycle.
          flowdockListener(user);
