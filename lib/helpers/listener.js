    ...
    function listeningStream(cb) {
      ...
      es.onmessage = function(e) {
        ...
        if(message.event === "message" || message.event === "comment") {
          ...
          // Unknown user == restart the loop
          if(!authorData) {
            console.log("Got a new user in the flow... restarting.");
            es.close();
          ...
          if(!message.flow) {
            ...
            // If the author of this notification is the current user, we need to reverse authorData.
            var oneToOneRecipient = (authorData.id.toString() === user.id.toString() ? users[message.to] : authorData);
            flowData = {
              name: '@' + (oneToOneRecipient.name || oneToOneRecipient.nick),
              web_url: 'https://www.flowdock.com/app/private/' + message.to,
          ...
          pushNotification(user, content, authorData, flowData);
        }
      };
